...
// 'CubicInOut', 'BounceOut', 'ExpoOut', 'ExpoIn', 'BackIn',
// 'BackOut', 'BackInOut'
...
export default function  ({ easeType, initValue, endValue, duration }) {
  switch (easeType) {
    ...
    case 'BounceOut':
      return t => {
        if ((t /= duration) < (1 / 2.75)) {
          ...
          return (endValue - initValue) * (7.5625 * (t -= (1.5 / 2.75)) * t + 0.75) + initValue
        } else if (t < (2.5 / 2.75)) {
          return (endValue - initValue) * (7.5625 * (t -= (2.25 / 2.75)) * t + 0.9375) + initValue
        } else {
          return (endValue - initValue) * (7.5625 * (t -= (2.625 / 2.75)) * t + 0.984375) + initValue
        }
      }
    ...
    case 'BackIn':
      return t => {
        const s = 1.70158
        return (endValue - initValue)*(t/=duration) * t * ((s+1)*t - s) + initValue
      }
 
    case 'BackOut':
      return t => {
        const s = 1.70158
        return (endValue - initValue)*((t=t/duration - 1) * t * ((s+1)*t + s) + 1) + initValue
      }
